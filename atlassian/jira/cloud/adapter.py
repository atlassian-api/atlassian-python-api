...
class JiraAdapter(CloudJira):
    ...
    def __init__(self, url: str, *args: Any, **kwargs: Any):
        ...
        # Map of legacy method names to new method names
        self._legacy_method_map = {
            "jql": "search_issues",
            "issue": "get_issue",
            "project": "get_project",
            "get_all_dashboards": "get_dashboards",
            "add_user_to_group": "add_group_member",
            "remove_user_from_group": "remove_group_member",
            # Add more mappings as needed
        }

        # Warn about deprecation on initialization
        warnings.warn(
            "JiraAdapter is deprecated. Please use the Jira class directly with the updated method names.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name in self._mapped_methods:
            ...
            if new_name != name:  # Only show warning if name actually changed
                warnings.warn(
                    f"Method '{name}' is deprecated, use '{new_name}' instead.", DeprecationWarning, stacklevel=2
                )
    ...
    def get_issue_remote_links(self, issue_id_or_key: str, global_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Legacy method to get remote links for an issue.
    ...
    def issue(self, issue_id_or_key):
        """
        Get an issue by ID or key (legacy method).

        Args:
            issue_id_or_key: ID or key of the issue to retrieve

        Returns:
            Dict containing issue details
        """
        warnings.warn("Method 'issue' is deprecated, use 'get_issue' instead.", DeprecationWarning, stacklevel=2)
        return self.get_issue(issue_id_or_key)
