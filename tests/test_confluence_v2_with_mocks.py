...
class TestConfluenceV2WithMocks(unittest.TestCase):
    ...
    def mock_response_for_endpoint(self, endpoint, params=None, status_code=200, mock_data=None):
        ...
        if "_links" in mock_data and "next" in mock_data["_links"]:
            self.mock_response.headers = {"Link": f'<{mock_data["_links"]["next"]}>; rel="next"'}
    ...
    def test_get_pages_with_pagination(self):
        ...
        page_data = {
            "results": [
                {"id": "123456", "title": "First Page", "status": "current", "spaceId": "789012"},
                {"id": "345678", "title": "Second Page", "status": "current", "spaceId": "789012"},
            ],
            "_links": {"self": "https://example.atlassian.net/wiki/api/v2/pages"},
        }
    ...
    def test_error_handling_validation(self):
        ...
        with self.assertRaises(HTTPError) as context:
            self.confluence.create_page(
                space_id="789012", title="", body="<p>Content</p>"  # Empty title, should cause validation error
            )
    ...
    def test_create_page_property(self):
        ...
        # Mock the response
        expected_data = self.mock_response_for_endpoint(endpoint, mock_data=PROPERTY_MOCK)

        # Call the method
        result = self.confluence.create_page_property(page_id, property_key, property_value)
    ...
    def test_get_page_labels(self):
        ...
        mock_data = {"results": [{"name": "test-label", "id": "label1"}, {"name": "another-label", "id": "label2"}]}
    ...
    def test_add_page_label(self):
        ...
        # Mock the response
        expected_data = self.mock_response_for_endpoint(endpoint, mock_data=LABEL_MOCK)
    ...
    def test_create_page_inline_comment(self):
        ...
        # Call the method
        result = self.confluence.create_page_inline_comment(page_id, body, inline_comment_properties)
    ...
    def test_create_whiteboard(self):
        ...
        # Mock the response
        expected_data = self.mock_response_for_endpoint(endpoint, mock_data=WHITEBOARD_MOCK)

        # Call the method
        result = self.confluence.create_whiteboard(space_id=space_id, title=title, template_key=template_key)
    ...
    def test_create_custom_content(self):
        ...
        # Mock the response
        expected_data = self.mock_response_for_endpoint(endpoint, mock_data=CUSTOM_CONTENT_MOCK)

        # Call the method
        result = self.confluence.create_custom_content(type=content_type, title=title, body=body, space_id=space_id)
    ...
    def test_search_with_pagination(self):
        ...
        search_data = {
            ...
            "_links": {"self": "https://example.atlassian.net/wiki/api/v2/search"},
        }
