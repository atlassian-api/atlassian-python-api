...
class TestConfluenceV2(unittest.TestCase):
    ...
    def setUp(self):
        self.confluence_v2 = ConfluenceV2(url="https://example.atlassian.net", username="username", password="password")
    ...
    def test_get_pages(self, mock_get_paged):
        ...
        mock_get_paged.assert_called_once_with(
            "api/v2/pages", params={"limit": 25, "status": "current", "body-format": "none"}
        )
    ...
    def test_create_page(self, mock_post):
        ...
        response = self.confluence_v2.create_page(
            space_id="SPACE123", title="New Page", body="<p>This is the content</p>", body_format="storage"
        )
        ...
        expected_data = {
            ...
            "status": "current",
            "title": "New Page",
            "body": {"storage": {"value": "<p>This is the content</p>"}},
        }
    ...
    def test_create_page_with_parent(self, mock_post):
        ...
        expected_data = {
            ...
            "status": "current",
            "title": "New Child Page",
            "body": {"storage": {"value": "<p>This is a child page</p>"}},
            "parentId": "PARENT123",
        }
        mock_post.assert_called_once_with("api/v2/pages", data=expected_data)
        self.assertEqual(response, mock_response)
    ...
    def test_create_page_with_wiki_format(self, mock_post):
        ...
        response = self.confluence_v2.create_page(
            space_id="SPACE123", title="Wiki Page", body="h1. Wiki Heading", body_format="wiki", representation="wiki"
        )
        ...
        expected_data = {
            ...
            "status": "current",
            "title": "Wiki Page",
            "body": {"wiki": {"value": "h1. Wiki Heading", "representation": "wiki"}},
        }
        mock_post.assert_called_once_with("api/v2/pages", data=expected_data)
        self.assertEqual(response, mock_response)
    ...
    def test_create_page_invalid_body_format(self):
        ...
        with self.assertRaises(ValueError):
            self.confluence_v2.create_page(
                space_id="SPACE123", title="Test Page", body="Test content", body_format="invalid"
            )
    ...
    def test_create_page_invalid_status(self):
        ...
        with self.assertRaises(ValueError):
            self.confluence_v2.create_page(
                space_id="SPACE123", title="Test Page", body="Test content", status="invalid"
            )
    ...
    def test_update_page(self, mock_put, mock_get_page):
        ...
        # Call the method
        response = self.confluence_v2.update_page(page_id="123", title="Updated Page", body="<p>Updated content</p>")

        # Assertions
        expected_data = {
            "id": "123",
            "title": "Updated Page",
            "version": {"number": 2, "message": "Updated via Python API"},
    ...
    def test_update_page_with_explicit_version(self, mock_put):
        ...
        response = self.confluence_v2.update_page(
            page_id="123", title="Updated Page", version=4  # Explicitly set version
        )
        ...
        expected_data = {
            "id": "123",
            "title": "Updated Page",
            "version": {"number": 5, "message": "Updated via Python API"},
        }
        mock_put.assert_called_once_with("api/v2/pages/123", data=expected_data)
        self.assertEqual(response, mock_response)
    ...
    def test_update_page_status(self, mock_put):
        ...
        response = self.confluence_v2.update_page(page_id="123", status="archived", version=1)
    ...
    def test_update_page_invalid_body_format(self):
        # Test invalid body_format
        with self.assertRaises(ValueError):
            self.confluence_v2.update_page(page_id="123", body="Test content", body_format="invalid")

    def test_update_page_invalid_status(self):
        # Test invalid status
        with self.assertRaises(ValueError):
            self.confluence_v2.update_page(page_id="123", status="invalid")
    ...
    def test_search(self, mock_get):
        ...
        # Assertions
        mock_get.assert_called_once_with("api/v2/search", params={"limit": 25, "query": "test query"})
        self.assertEqual(response, mock_response)
    ...
    def test_search_with_cql(self, mock_get):
        ...
        response = self.confluence_v2.search(
            query="", cql="type = 'page' AND space.id = '123'", limit=10, excerpt=False
        )

        # Assertions
        mock_get.assert_called_once_with(
            "api/v2/search", params={"limit": 10, "cql": "type = 'page' AND space.id = '123'", "excerpt": "false"}
        )
        self.assertEqual(response, mock_response)
    ...
    def test_search_content(self, mock_search):
        ...
        response = self.confluence_v2.search_content(
            query="test", type="page", space_id="SPACE123", status="current", limit=10
        )
    ...
    def test_get_page_properties_with_cursor(self, mock_get_paged):
        ...
        response = self.confluence_v2.get_page_properties(page_id="PAGE123", cursor="next-page-cursor", limit=10)

        # Assertions
        mock_get_paged.assert_called_once_with(
            "api/v2/pages/PAGE123/properties", params={"limit": 10, "cursor": "next-page-cursor"}
        )
        self.assertEqual(response, mock_properties)
    ...
    def test_create_page_property(self, mock_post):
        ...
        response = self.confluence_v2.create_page_property(
            page_id="PAGE123", property_key="test.prop", property_value={"data": "test"}
        )

        # Assertions
        expected_data = {"key": "test.prop", "value": {"data": "test"}}
    ...
    def test_create_page_property_invalid_key(self):
        ...
        with self.assertRaises(ValueError):
            self.confluence_v2.create_page_property(
                page_id="PAGE123", property_key="invalid-key!", property_value="test"
            )
    ...
    def test_update_page_property(self, mock_put, mock_get_property):
        ...
        response = self.confluence_v2.update_page_property(
            page_id="PAGE123", property_key="prop1", property_value="updated"
        )
    ...
    def test_update_page_property_with_explicit_version(self, mock_put):
        ...
        # Call the method with explicit version
        response = self.confluence_v2.update_page_property(
            page_id="PAGE123", property_key="prop1", property_value="updated", version=4  # Explicitly set version
        )
    ...
    def test_get_page_labels_with_filters(self, mock_get_paged):
        ...
        response = self.confluence_v2.get_page_labels(
            page_id="PAGE123", prefix="team-", cursor="next-page-cursor", limit=10
        )

        # Assertions
        mock_get_paged.assert_called_once_with(
            "api/v2/pages/PAGE123/labels", params={"limit": 10, "prefix": "team-", "cursor": "next-page-cursor"}
        )
        self.assertEqual(response, mock_labels)
    ...
    def test_create_whiteboard(self, mock_post):
        ...
        result = self.confluence_v2.create_whiteboard(
            space_id=space_id, title=title, parent_id=parent_id, template_key=template_key, locale=locale
        )

        mock_post.assert_called_with("api/v2/whiteboards", data=expected_data)
    ...
    def test_delete_whiteboard(self, mock_delete):
        ...
        mock_delete.assert_called_with("api/v2/whiteboards/123456")
    ...
    def test_get_whiteboard_children(self, mock_get_paged):
        ...
        mock_get_paged.return_value = [{"id": "child1", "title": "Child 1"}, {"id": "child2", "title": "Child 2"}]
    ...
    def test_get_whiteboard_ancestors(self, mock_get):
        ...
        whiteboard_id = "123456"
        mock_response = {"results": [{"id": "parent1", "type": "whiteboard"}, {"id": "parent2", "type": "space"}]}
        mock_get.return_value = mock_response

        result = self.confluence_v2.get_whiteboard_ancestors(whiteboard_id)

        mock_get.assert_called_with("api/v2/whiteboards/123456/ancestors")
    ...
    def test_create_custom_content(self, mock_post):
        ...
        result = self.confluence_v2.create_custom_content(
            type=content_type, title=title, body=body, space_id=space_id, page_id=page_id
        )
    ...
    def test_get_custom_content_by_id(self, mock_get):
        ...
        result = self.confluence_v2.get_custom_content_by_id(
            custom_content_id=custom_content_id, body_format=body_format
        )

        mock_get.assert_called_with("api/v2/custom-content/123456", params={"body-format": body_format})
    ...
    def test_get_custom_content(self, mock_get_paged):
        ...
        result = self.confluence_v2.get_custom_content(
            type=content_type, space_id=space_id, page_id=page_id, status=status, sort=sort, limit=limit
        )

        mock_get_paged.assert_called_with("api/v2/custom-content", params=expected_params)
    ...
    def test_update_custom_content(self, mock_put):
        ...
        mock_put.return_value = {"id": custom_content_id, "title": title, "version": {"number": version_number}}
        ...
        mock_put.assert_called_with(f"api/v2/custom-content/{custom_content_id}", data=expected_data)
    ...
    def test_get_custom_content_children(self, mock_get_paged):
        ...
        result = self.confluence_v2.get_custom_content_children(
            custom_content_id=custom_content_id, cursor=cursor, limit=limit
        )

        mock_get_paged.assert_called_with(
            f"api/v2/custom-content/{custom_content_id}/children", params={"cursor": cursor, "limit": limit}
        )
    ...
    def test_get_custom_content_ancestors(self, mock_get):
        ...
        custom_content_id = "123456"
        mock_response = {"results": [{"id": "parent1", "type": "page"}, {"id": "parent2", "type": "space"}]}
        mock_get.return_value = mock_response

        result = self.confluence_v2.get_custom_content_ancestors(custom_content_id)

        mock_get.assert_called_with(f"api/v2/custom-content/{custom_content_id}/ancestors")
    ...
    def test_add_custom_content_label(self, mock_post):
        ...
        expected_data = {"name": label, "prefix": prefix}

        mock_post.return_value = {"id": "label1", "name": label, "prefix": prefix}

        result = self.confluence_v2.add_custom_content_label(
            custom_content_id=custom_content_id, label=label, prefix=prefix
        )

        mock_post.assert_called_with(f"api/v2/custom-content/{custom_content_id}/labels", data=expected_data)
    ...
    def test_delete_custom_content_label(self, mock_delete):
        ...
        self.confluence_v2.delete_custom_content_label(custom_content_id=custom_content_id, label=label, prefix=prefix)

        mock_delete.assert_called_with(
            f"api/v2/custom-content/{custom_content_id}/labels", params={"name": label, "prefix": prefix}
        )
    ...
    def test_get_custom_content_properties(self, mock_get_paged):
        ...
        result = self.confluence_v2.get_custom_content_properties(
            custom_content_id=custom_content_id, sort=sort, limit=limit
        )

        mock_get_paged.assert_called_with(
            f"api/v2/custom-content/{custom_content_id}/properties", params={"sort": sort, "limit": limit}
        )
    ...
    def test_get_custom_content_property_by_key(self, mock_get):
        ...
        mock_response = {"id": "prop1", "key": property_key, "value": {"test": "value"}, "version": {"number": 1}}
        mock_get.return_value = mock_response
        ...
        mock_get.assert_called_with(f"api/v2/custom-content/{custom_content_id}/properties/{property_key}")
    ...
    def test_create_custom_content_property(self, mock_post):
        ...
        mock_post.assert_called_with(f"api/v2/custom-content/{custom_content_id}/properties", data=expected_data)
    ...
    def test_update_custom_content_property(self, mock_put):
        ...
        mock_put.assert_called_with(
            f"api/v2/custom-content/{custom_content_id}/properties/{property_key}", data=expected_data
        )
    ...
    def test_delete_custom_content_property(self, mock_delete):
        ...
        self.confluence_v2.delete_custom_content_property(custom_content_id=custom_content_id, key=property_key)

        mock_delete.assert_called_with(f"api/v2/custom-content/{custom_content_id}/properties/{property_key}")
