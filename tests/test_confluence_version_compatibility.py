...
class TestConfluenceVersionCompatibility(unittest.TestCase):
    ...
    def test_v1_and_v2_method_availability(self):
        ...
        for method_name in key_methods:
            ...
            self.assertTrue(hasattr(self.confluence_v1, method_name), 
                           f"Method {method_name} not found in v1 API")
            self.assertTrue(hasattr(self.confluence_v2, method_name), 
                           f"Method {method_name} not found in v2 API")
        ...
        for method_name in compat_methods:
            self.assertTrue(hasattr(self.confluence_v2, method_name),
                           f"Compatibility method {method_name} not found in v2 API")
    ...
    def test_get_page_by_id_compatibility(self):
        ...
        self.assertEqual(
            v1_result["body"]["storage"]["value"], 
            v2_result["body"]["storage"]["value"]
        )
    ...
    def test_create_page_compatibility(self):
        ...
        # Call methods on both API versions
        v1_result = self.confluence_v1.create_page(
            space=space_key,
            title=title,
            body=body
        )
        ...
        v2_result = self.confluence_v2.create_page(
            space_id=space_id,  # v2 uses space_id instead of space_key
            title=title,
            body=body
        )
    ...
    def test_properties_compatibility(self):
        ...
        v1_response = {
            ...
            "size": 2,
            "_links": {
                "self": f"https://example.atlassian.net/wiki/rest/api/content/{content_id}/property"
            }
        }
        self.mock_response_v1.status_code = 200
        self.mock_response_v1.text = json.dumps(v1_response)
    ...
    def test_labels_compatibility(self):
        ...
        v1_response = {
            ...
            "size": 2,
            "_links": {
                "self": f"https://example.atlassian.net/wiki/rest/api/content/{content_id}/label"
            }
        }
        self.mock_response_v1.status_code = 200
        self.mock_response_v1.text = json.dumps(v1_response)
