...
def example_get_page_by_id():
    ...
    try:
        ...
        page_with_body = confluence_v2.get_page_by_id(
            page_id, 
            body_format="storage", 
            expand=["version"]
        )
...
def example_get_pages():
    ...
    try:
        ...
        pages = confluence_v2.get_pages(
            space_id=space_id,
            limit=10,
            sort="-modified-date"  # Most recently modified first
        )
        ...
        title_pages = confluence_v2.get_pages(
            space_id=space_id,
            title="Meeting Notes",  # Pages with this exact title
            limit=5
        )
...
def example_get_child_pages():
    ...
    try:
        ...
        child_pages = confluence_v2.get_child_pages(
            parent_id=parent_id,
            sort="child-position"
        )
...
def example_search():
    ...
    try:
        ...
        print("\nSearch with CQL:")
        cql_results = confluence_v2.search(
            query="",
            cql="type = 'page' AND created > startOfMonth(-1)",
            limit=5
        )
...
def example_search_content():
    ...
    try:
        ...
        blog_results = confluence_v2.search_content(
            query="",  # Empty query to match any content
            type="blogpost",
            status="current",
            limit=3
        )
...
def example_get_spaces():
    ...
    try:
        ...
        global_spaces = confluence_v2.get_spaces(
            type="global",
            status="current",
            limit=5
        )
        ...
        labeled_spaces = confluence_v2.get_spaces(
...
def example_get_space_by_id():
    ...
    try:
        ...
        content = confluence_v2.get_space_content(
            space_id=space_id,
            sort="-modified",
            limit=5
        )
        ...
        for i, item in enumerate(content[:3]):
            ...
            print(f"{i+1}. {content_item.get('title', 'Unknown')} "
