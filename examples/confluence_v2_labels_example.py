...
def add_page_labels_example(page_id):
    ...
    try:
        # Add a single label
        single_label = confluence.add_page_label(page_id=page_id, label="example-label")

        print(f"Added label: {single_label.get('name', 'unknown')}")

        # Add multiple labels at once
        multiple_labels = confluence.add_page_labels(
            page_id=page_id, labels=["test-label-1", "test-label-2", "example-api"]
        )
...
def manage_space_labels_example(space_id):
    ...
    try:
        # Add a single label
        single_label = confluence.add_space_label(space_id=space_id, label="space-example")

        print(f"Added label: {single_label.get('name', 'unknown')}")

        # Add multiple labels at once
        multiple_labels = confluence.add_space_labels(space_id=space_id, labels=["space-test-1", "space-test-2"])
